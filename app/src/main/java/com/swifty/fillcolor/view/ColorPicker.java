...
public class ColorPicker extends View {
    ...
    public void setOnChangedListener(OnColorChangedListener onChangedListener) {
...
//        canvas.drawPath(colorViewPath, colorViewPaint);
    ...
    protected void onSizeChanged(int width, int height, int oldw, int oldh) {
        ...
        valueSliderPath.arcTo(outerWheelRect, 288, 162);
        valueSliderPath.arcTo(innerWheelRect, 90, -162);
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (d <= colorWheelRadius) {
                    ...
                    if (onChangedListener != null) {
                        onChangedListener.colorChangedListener(getColor());
                    }
                    invalidate();

                } else if (x >= getWidth() / 2 && d >= innerWheelRadius) {
                    float light = (float) Math.max(0, Math.min(1, Math.atan2(cy, cx) / Math.PI + 0.5f));
                    colorHSV[2] = light > 0.1 ? light : (float) 0.1;
                    if (onChangedListener != null) {
                        onChangedListener.colorChangedListener(getColor());
                    }
                    invalidate();
                }
                L.e("color2", colorHSV[0] + ";" + colorHSV[1] + ";" + colorHSV[2]);
                return true;
        }
